...
function setup_localrc() {
    ...
    sudo cp -f $JENKINS_TEST_HOME/devstack-gate/$DEVSTACK_GATE_FEATURE_MATRIX $BASE/new/devstack-gate/ 
    MY_ENABLED_SERVICES=`cd $BASE/new/devstack-gate && ./test-matrix.py -b $LOCALRC_BRANCH -f $DEVSTACK_GATE_FEATURE_MATRIX`
    echo "enable_services in localrc for devstack " $MY_ENABLED_SERVICES
